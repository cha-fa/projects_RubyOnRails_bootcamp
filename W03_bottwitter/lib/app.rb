...
def say_hello_to_five(handles, client)
  random_five = handles.sample(5)
  random_five.each do |journalist|
    client.update("Hey #{journalist}, les moussaillons de @the_hacking_pro te saluent ! #bonjour_monde")
  end
end
...
def like_hello(client)
  ...
  client.fav(tweets_to_like)
  return tweets_to_like
end



def follow_hello(client)
  tweet_to_follow = client.search("#bonjour_monde", :result_type => "recent").take(20)
  account_to_follow = [ ]
  tweet_to_follow.each do |tweet| 
    account_to_follow << tweet.user.screen_name
  end
  account_to_follow.each do |account|
...
client_streaming = login_twitter_streaming

def like_live (client, client_streaming)
...
like_live(client, client_streaming)
